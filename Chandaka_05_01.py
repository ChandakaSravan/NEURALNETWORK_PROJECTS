...
class DCGAN(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as gtape, tf.GradientTape() as dtape:
            ...
            real_images = self.discriminator(data, training=True)
            fake_images = self.discriminator(generated_images, training=True)

            # calculating the loss for discriminator
            real_labels = tf.ones_like(real_images)
            fake_labels = tf.zeros_like(fake_images)
            r_loss = self.loss_fn(real_labels, real_images)
            f_loss = self.loss_fn(fake_labels, fake_images)
            d_loss = r_loss*0.5 + f_loss*0.5

            # calculating the loss for generator
            generated_labels = tf.ones_like(fake_images)
            g_loss = self.loss_fn(generated_labels, fake_images)

        gradients_of_generator = gtape.gradient(g_loss, self.generator.trainable_variables)
        gradients_of_discriminator = dtape.gradient(d_loss, self.discriminator.trainable_variables)
